...
public extension MZTableLayoutDelegate {
    /// 默认配置（0，0），不冻结
    func frozenUnit(_ colletionView: UICollectionView) -> (Int, Int) {
        return (0, 0)
    }
...
public class MZTableLayout: UICollectionViewLayout {
    ...
    override public func prepare() {
        ...
        for i in 0..<scale.0 {
            let curCellWidth: CGFloat = delegate?.baseWidth(collectionView!, cellWidthOf: i) ?? 0
            cellWidth += curCellWidth
        }
        
        var cellHeight: CGFloat = 0
        for i in 0..<scale.1 {
            let curCellHeight: CGFloat = delegate?.baseHeight(collectionView!, cellHeightOf: i) ?? 0
            cellHeight += curCellHeight
        }

        //根据规模和每行每列的宽高，可以确定content size的大小
        contentSize = CGSize.init(width: cellWidth + CGFloat(scale.0 - 1) * itemSpacing + 2 * itemMargin,
                                  height: cellHeight + CGFloat(scale.1 - 1) * itemSpacing + 2 * itemMargin)
        ...
        for index in 0..<self.collectionView!.numberOfItems(inSection: 0) {
            ...
            while !isFind && endFlag == 0 {
                ...
                if !isFind_single {
                    ...
                    isFind = true;
                    //生成位置属性，保存
                    var atWidthPerCell: CGFloat = 0
                    for i in 0..<tempX_single {
                        let curCellWidth: CGFloat = delegate?.baseWidth(collectionView!, cellWidthOf: i) ?? 0
                        atWidthPerCell += curCellWidth
                    }
                    var atWidth = atWidthPerCell + CGFloat(tempX_single) * itemSpacing + itemMargin;
                    
                    var atHeightPerCell: CGFloat = 0
                    for i in 0..<tempY_single {
                        let curCellHeight: CGFloat = delegate?.baseHeight(collectionView!, cellHeightOf: i) ?? 0
                        atHeightPerCell += curCellHeight
                    }
                    var atHeight = atHeightPerCell + CGFloat(tempY_single) * itemSpacing + itemMargin;
                    ...
                    for i in (tempX_single)..<(tempX_single + curItemScale!.0) {
                        let curCellWidth: CGFloat = delegate?.baseWidth(collectionView!, cellWidthOf: i) ?? 0
                        itemWidth += curCellWidth
                    }
                    itemWidth += CGFloat(curItemScale!.0 - 1) * itemSpacing
            
                    var itemHeight: CGFloat = 0
                    for i in (tempY_single)..<(tempY_single + curItemScale!.1) {
                        let curCellHeight: CGFloat = delegate?.baseHeight(collectionView!, cellHeightOf: i) ?? 0
                        itemHeight += curCellHeight
                    }
                    itemHeight += CGFloat(curItemScale!.1 - 1) * itemSpacing
